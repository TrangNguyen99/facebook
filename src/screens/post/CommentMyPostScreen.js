...
import {
  getCommentMyPostRequest,
  getMoreCommentMyPostRequest,
  getMyPostById,
  setCommentMyPostRequest,
} from '../../slices/postSlice';
import * as colors from './../../constants/colors';

const spinValue = new Animated.Value(0);

function CommentMyPostScreen({route}) {
  ...
  const listCommentMyPostGetById = useSelector(
    (state) => state.post.listCommentMyPostGetById,
  );
  const loadingGetCommentMyPostRequest = useSelector(
    (state) => state.post.loadingGetCommentMyPostRequest,
  );
  const loadingGetMoreCommentMyPostRequest = useSelector(
    (state) => state.post.loadingGetMoreCommentMyPostRequest,
  );
  ...
  if (loadingGetCommentMyPostRequest) {
    return (
      <View style={styles.container}>
        <CommentHeader
          like={myPostGetById.like}
          isLiked={parseInt(myPostGetById.is_liked, 10) === 1}
        />
        <ScrollView>
          <View
            style={{
  ...
  return (
    <View style={styles.container}>
      ...
      {network ? (
        <>
          {listCommentMyPostGetById.length ? (
            <>
              {loadingGetMoreCommentMyPostRequest ? (
                <ActivityIndicator
                  size="small"
              ...
              <ListComment listComment={listCommentMyPostGetById} />
...
export default CommentMyPostScreen;
