...
import {editPostRequest, getPostById} from '../../slices/postSlice';
...
function EditPostScreen({navigation, route}) {
  ...
  let disabled = true;
  if (show) {
    if (feelState !== postGetById.state) {
      disabled = false;
    } else if (described !== postGetById.described) {
      disabled = false;
    }

    if (disabled) {
      if (postGetById.image) {
        if (images.length !== postGetById.image.length) {
          disabled = false;
        } else {
          for (let i = 0; i < images.length; ++i) {
            if (images[i].uri !== postGetById.image[i].url) {
              disabled = false;
              break;
            }
          }
    ...
    if (disabled) {
      if (postGetById.video) {
        if (video.uri !== postGetById.video.url) {
          disabled = false;
        }
      } else {
        if (video) {
      ...
      editPostRequest({
        ...
        video:
          mediaStatus === 'video'
            ? {
                url: video.uri,
                thumb: postGetById.video ? postGetById.video.thumb : null,
              }
            : '',
      }),
    );
  ...
  useEffect(() => {
    if (postGetById?.id === route.params.postId && !show) {
      setDescribed(postGetById.described);
      setFeelState(postGetById.state);
      if (postGetById.image) {
        setImages(
          postGetById.image.map((value) => {
      ...
      if (postGetById.video) {
        setVideo({uri: postGetById.video.url});
        setMediaStatus('video');
      }
      setShow(true);
    }
          ...
          <ScrollView style={styles.top}>
            ...
            {mediaStatus === 'video' && (
              <>
                ...
                <VideoPlayer
                  video={video}
                  thumbnail={{uri: postGetById.video.thumb}}
                  videoWidth={1600}
                  videoHeight={900}
                />
              </>
